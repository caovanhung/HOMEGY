...
uint8_t save_flash()
{
	...
	while(1)
	{
        ...
        if (err != ESP_OK){
            ...
            return -1;  // Lỗi khi xóa bộ nhớ flash
        }
        ...
        while (1)
        {
            ...
            if (uart_read_bytes(UART_NUM_0, &data, 1, 100) == 1){
                ...
                if (err != ESP_OK){
                    ...
                    return -1;  // Lỗi khi ghi vào bộ nhớ flash
                }
        ...
        if (err != ESP_OK){
            ...
            return -1;  // Lỗi khi ghi ký tự kết thúc vào bộ nhớ flash
        }
...
uint8_t load_flash(uint8_t idl, uint8_t idh, uint8_t index)
{
	uint8_t data[6];
	uint16_t i;
	...
	if(id == 65535){
		for(i=0; i<4096; i++)
		{
			...
			if (err != ESP_OK)
			{
				return -1;  // Lỗi thao tác với flash
			}
...
uint8_t clear_flash(uint8_t idl, uint8_t idh)
{
	esp_err_t err;  // Biến kiểm tra lỗi cho các thao tác bộ nhớ
	uint16_t id = idl + 256*idh;
	if(id < 65535){
...
uint8_t load_custom_remote(uint8_t index)
{
	...
	int64_t start_time, current_time;  // Biến lưu thời gian để kiểm tra timeout
	esp_err_t err;
	if((!params[index+3]) || (memory[params[index+1]+256*params[index+2]] != 2) || (params[index+1]+256*params[index+2] < 21)){
		return 0;
	}
	while(1)
	{
...
uint8_t delete_action(uint8_t idl, uint8_t idh, uint8_t index)
{
	uint16_t i;
	uint8_t* cache = (uint8_t*)malloc(4096);
	esp_err_t err = esp_partition_read_raw(partition, 4096, cache, 4096);
...
uint8_t set_status_script(uint8_t idl, uint8_t idh, uint8_t status)
{
	uint16_t i;
	uint8_t* cache = (uint8_t*)malloc(4096);
	esp_err_t err = esp_partition_read_raw(partition, 4096, cache, 4096);
...
uint8_t save_status_remote(uint8_t idl, uint8_t idh, uint8_t index, uint8_t temp, uint8_t mode, uint8_t fan, uint8_t swing)
{
	if(!check_list_remote(idl, idh, index)) return -1;
	uint16_t i;
	uint8_t cache[480];
...
uint8_t set_position_remote(uint8_t idl, uint8_t idh, uint8_t index, uint8_t* position)
{
	uint16_t i;
	uint8_t cache[480];
	esp_err_t err = esp_partition_read_raw(partition, 0, cache, 480);
...
uint8_t load_position_remote(uint8_t idl, uint8_t idh, uint8_t index, uint8_t *position)
{
	uint16_t i;
	uint8_t data[24];
	esp_err_t err;
...
uint8_t find_power_custom(uint8_t index)
{
	if(remote_param_ir == NULL) return -1;
	uint64_t one = 1;
	uint8_t i;
	uint8_t j;
	uint8_t k = 0;
...
uint8_t find_power_raw(uint8_t index)
{
	if((memory[params[index+1]+256*params[index+2]] != 4) || (params[index+1]+256*params[index+2]>20) || (params[index+1]+256*params[index+2]<2) || (find_type_raw(params[index+1], params[index+2]) != 2)) {
		return 0;
	}
